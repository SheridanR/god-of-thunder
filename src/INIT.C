...
int16_t initialize(void){
    ...
    if(res_read("RANDOM",(char*)rnd_array)<0) return 15;
    if(res_read("DEMO",(char*)demo_key)<0) return 16;
    if(res_read("TEXT",(char*)text)<0) return 2;
    if(res_read("ODINPIC",(char*)odin)<0) return 12;
    if(res_read("HAMPIC",(char*) hampic)<0) return 13;
...
void exit_code(int16_t ex_flag){
    ...
    // it being the new millenium, this is no longer necessary
    /*
    in.x.ax=0x0003;
    int86(0x10,&in,&out);
    */
...
void story(void){
    ...
    xshowpage(38400u);
    if(res_read("OPENSONG",(char*)song)<0) return;
    if(res_read("STORY1",(char*)tmp_buff)<0) return;
    if(res_read("OPENBACK",(char*)back)<0) return;
    if(res_read("STORYPIC",(char*)back)<0) return;
    p=tmp_buff;
    pg=0u;
